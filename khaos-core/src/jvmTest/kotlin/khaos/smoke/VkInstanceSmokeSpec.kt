...
/** This module's directory. Tests run with the working directory pinned to the repo root. */
private val moduleRoot = "$projectRoot/khaos-core"

/** This spec's own source, read by the TCs that assert on the code they are part of. */
private val ownSource = "$moduleRoot/src/jvmTest/kotlin/khaos/smoke/VkInstanceSmokeSpec.kt"
...
class VkInstanceSmokeSpec : FunSpec({
    ...
    test("TC-10: smoke test scope limited to VkInstance and VkDevice") {
        val source = File(ownSource).readText()
    ...
    test("TC-17: LWJGL natives are platform-conditional in build.gradle.kts") {
        val content = File("$moduleRoot/build.gradle.kts").readText()
